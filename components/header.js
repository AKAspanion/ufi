        ...
        <div className={styles.container}>
              ...
              <ul className={cs(styles.navbar__ul, styles.navbar__nav)}>
                {liMeta.map(({ name, id, link }) => (
                  <li
                    ...
                    {link !== '' ? (
                      <Link href={link} className={(styles.nav__link, styles.dropdown__toggle)}>
                        {name}
                      </Link>
                    ) : (
                      name
          ...
          <div className={styles.menu} onClick={() => setShowMenu(!showMenu)}>
            {showMenu ? (
              ...
              <HiOutlineMenuAlt4 size={32} color={isDark() ? 'black' : 'white'} />
            )}
          </div>
          <div
            className={cs([
                    ...
                    <Collapsible
                      ...
                      trigger={
                        <div key={id} className={styles.menu__top__title}>
                          {getNested(id).length === 0 ? <Link href={link}>{name}</Link> : name}
                          {getNested(id).length !== 0 && <BsChevronDown />}
                        </div>
                      }>
                      <div className={styles.menu__top__sub}>
                        {getNested(id).map(({ id, name, link: link2 = '#', submeta = [] }) => (
                          <Collapsible
                            key={id}
                            trigger={
                              <div key={id} className={styles.menu__sub__head}>
                                {submeta.length === 0 ? <Link href={link2}>{name}</Link> : name}
